...
public class SSSController {
	...
	@RequestMapping(value = "/mastProductListEach0")
	public String mastProductListEach0P(HttpSession session,Model model, @RequestParam(defaultValue = "1") int curPage) {
		String path = "";
		MemberDTO memberDTO = (MemberDTO) session.getAttribute("member");
		if (memberDTO != null && memberDTO.getMember_level() == 0) {
			sssssService.mastProductListEach022(model, curPage);
			path="/mast/mastProductListEach0";
		} else {
			path = "redirect:/";
		}
		return path;
	...
	@RequestMapping(value = "/mastProductListEach0EachFix", method = RequestMethod.POST)
	public String mastProductListEach0EachFix(HttpSession session,RedirectAttributes ra, int product_num, String productSize_size,
			String productEach_color, @RequestParam(defaultValue = "0") int productEach_each) {
		String path = "";
		MemberDTO memberDTO = (MemberDTO) session.getAttribute("member");
		if (memberDTO != null && memberDTO.getMember_level() == 0) {
			...
			ra.addFlashAttribute("message", sssssService.mastProductListEach0EachFix(productEachDTO));
			path="redirect:/mast/mastProductListEach0";
		} else {
			path = "redirect:/";
		}
		return path;
	...
	public String tttttteeeessss(HttpSession session, @RequestParam(required = false) Date first_date,
			@RequestParam(required = false) Date last_date, Model model) {
		String path = "";
		MemberDTO memberDTO = (MemberDTO) session.getAttribute("member");
		if (memberDTO != null && memberDTO.getMember_level() == 0) {
			Map<String, Object> map = new HashMap<>();
